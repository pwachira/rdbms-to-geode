 ...
 * @author Jeff Cherng
 */
public class ItemExtractor implements PayloadWrapperExtractor<Map, Item> {

	@Override
	public Item extractData(PayloadWrapper<Map> payloadWrapper) {
		Item value = null;
		if (payloadWrapper.hasPayload()) {
			value = new Item();
			Map payload = payloadWrapper.getPayload();
			value.setName((String) payload.get("name"));
			value.setDescription((String) payload.get("description"));
			value.setPrice(payload.get("price").toString());
		}
		return value;
	}
